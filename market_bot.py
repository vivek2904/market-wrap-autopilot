...
def get_valuation_and_summary():
    """Scrapes PE, cleaning quotes and formatting return forecasts."""
    ...
    except: return "Historical analysis for Nifty valuation.", "Analyzing...", "7.3%", ""

def fetch_analysis_data():
    """Calculates RSI & Trend for 170+ stocks."""
...
def build_report():
    ...
    html = f"""
    <style>
        .market-card {{ font-family: 'Inter', -apple-system, sans-serif; max-width: 950px; margin: auto; color: #1e293b; }}
    ...
    <div class="market-card">
        ...
        <div class="insight-box">
            <h2 style="margin-top:0;">📊 Market Valuation Analytics</h2>
            <div style="display:flex; gap:40px; margin:25px 0;">
                <div><small>CURRENT P/E</small><br><b style="font-size:28px;">{pe}</b></div>
                <div><small>1Y FORECAST</small><br><b style="font-size:28px; color:#22c55e;">{target_return}</b></div>
            </div>
            <p>{summary}</p>
            {v_table}
        </div>
...
def post():
    ...
    # CLICK RATE OPTIMIZATION: Dynamic Title & AIOSEO tags
    title = f"India Market Wrap: Nifty {change:+.2f}% | Sector Winners & 170+ Stock Analysis"
    seo_desc = f"Today's Nifty Analysis: Index ends at {change:+.2f}%. Explore technicals (RSI/Trend) for 170+ NSE stocks and valuation insights for Indian markets."
    ...
    print(f"✅ Success: {title}" if res.status_code == 201 else f"❌ Error: {res.text}")
